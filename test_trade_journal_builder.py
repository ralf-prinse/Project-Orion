...
def main():
    ...
    assert entries
    assert len(entries) == len(result.cycle_results[0].allocation.decisions)
    ...
    assert first.symbol
    assert first.action in {"OPEN_POSITION", "REJECTED"}
    assert first.decision in {"BUY", "HOLD", "SELL"}
    ...
    print("TRADE JOURNAL BUILDER: PASS ✅")
