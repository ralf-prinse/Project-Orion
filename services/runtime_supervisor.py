...
class RuntimeSupervisor:
    ...
    The supervisor never mutates TradingSession and never decides, sizes,
    opens, updates or closes trades. It is an operational observer only.
    """
    ...
    def runtime_started(self) -> None:
        ...
        self._record("RUNTIME_STARTED", "Continuous runtime started.")
    ...
    def iteration_completed(
        ...
        self.health.last_iteration_duration_seconds = duration_seconds
    ...
    def iteration_failed(
        ...
        self.health.last_iteration_duration_seconds = duration_seconds
        self.health.last_heartbeat_at = self.clock()
        self.health.last_error = repr(error) if error is not None else None
        self._record(
            "ITERATION_FAILED",
    ...
    def runtime_stopped(self, reason: str) -> None:
        now = self.clock()
        self.health.status = "STOPPED"
        self.health.stopped_at = now
        self.health.last_heartbeat_at = now
        self._record("RUNTIME_STOPPED", reason)
    ...
    def _update_session_counts(
        ...
        self.health.open_positions = len(session.portfolio.positions)
        self.health.position_states = len(session.position_states)
        self.health.risk_plans = len(session.risk_plans)
