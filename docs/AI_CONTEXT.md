...
Sprint 10.14 introduced the Event Bus Foundation. The new `core/events` package contains deterministic event models, an explicit EventRegistry and a synchronous EventBus.

The ScanOrchestrator may publish lifecycle events such as scan start, pipeline step start, pipeline step completion, pipeline failure and scan completion. AI remains an explanation layer only and must never subscribe to events in order to create investment decisions. Future AI components may only explain deterministic outputs generated by Orion.
